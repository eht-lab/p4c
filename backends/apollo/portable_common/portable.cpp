...
void PortableCodeGenerator::createStructLike(ConversionContext *ctxt, const IR::Type_StructLike *st,
    ...
    for (auto f : st->fields) {
        ...
        } else if (auto type = ftype->to<IR::Type_Enum>()) {
            field->append(f->name.name);
            field->append(type->width_bits());
            field->append(false);
            max_length += type->width_bits();
...
void ExternConverter_Counter::convertExternInstance(ConversionContext *ctxt,
    ...
    big_int sizeVal = sz->to<IR::Constant>()->value;
    ...
    // Tuna Counter supports 2 or 3 parameters:
    // - 2 parameters: Counter(n_counters, type) - traditional PSA mode
    // - 3 parameters: Counter(n_counters, type, index_mode) - Tuna extended mode
    size_t numParams = eb->getConstructorParameters()->size();
    if (numParams != 2 && numParams != 3) {
        modelError("%1%: expected two or three parameters for Counter", eb);
        return;
    }

    // Attribute 1: n_counters
    ...
    // Attribute 3: index_mode (Tuna exclusive, optional)
    if (numParams == 3) {
        ...
        auto param3 = eb->getConstructorParameters()->getParameter(2);
        auto modeMem = arg3->toString();
        LOG5("In convertParam with p3 " << param3->toString() << " and mem " << modeMem);
        auto modeJsn = ctxt->conv->convertParam(param3, modeMem);
        if (modeJsn != nullptr) {
            arr->append(modeJsn);
        } else {
            // convertParam failed, manually create index_mode attribute
            auto modeAttr = new Util::JsonObject();
            modeAttr->emplace("name"_cs, "index_mode");
            modeAttr->emplace("type"_cs, "string");
            modeAttr->emplace("value"_cs, modeMem);
            arr->append(modeAttr);
        }
    }
}
...
void ExternConverter_DirectCounter::convertExternInstance(UNUSED ConversionContext *ctxt,
    ...
    if (it == ctxt->structure->directCounterMap.end()) {
        ...
        // Direct Counter only has a single argument, which is tuna metadata
...
void ExternConverter_Meter::convertExternInstance(UNUSED ConversionContext *ctxt,
    ...
    cstring val = stringRepr(s->value, ROUNDUP(bitwidth, 8));
...
void ExternConverter_ActionProfile::convertExternInstance(UNUSED ConversionContext *ctxt,
    ...
    if (JsonObjects::find_object_by_name(ctxt->action_profiles, name)) return;
...
void ExternConverter_ActionSelector::convertExternInstance(UNUSED ConversionContext *ctxt,
    ...
    // Convert from tuna/PNA names to v1model names
    ...
    if (input == nullptr) {
        ...
        ::P4::warning(ErrorType::WARN_UNUSED,
                      ...
                      "and cannot be included in tuna JSON",
                      c);
        return;
    }
