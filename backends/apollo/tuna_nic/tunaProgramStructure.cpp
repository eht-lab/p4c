...
#include "tunaProgramStructure.h"

namespace P4 {

using namespace P4::literals;

void InspectTunaProgram::postorder(const IR::Declaration_Instance *di) {
...
void InspectTunaProgram::addHeaderType(const IR::Type_StructLike *st) {
    ...
    for (auto f : type->fields) {
        ...
        if (ft->is<IR::Type_StructLike>()) {
            if (auto hft = ft->to<IR::Type_Header>()) {
                ...
                // header_union_types already added in addHeaderType(), don't add again
            ...
            if (ft->is<IR::Type_Bits>()) {
                ...
                auto enumType = ft->to<IR::Type_Enum>();
                auto memberCount = enumType->members.size();
                unsigned enumWidth = memberCount <= 2    ? 1
                                     : memberCount <= 4  ? 2
                                     : memberCount <= 8  ? 3
                                     : memberCount <= 16 ? 4
                                                         : 8;
                LOG5("Enum " << enumType->name << " has " << memberCount << " members, using "
                             << enumWidth << " bits");
                pinfo->scalars_width += enumWidth;
                pinfo->scalarMetadataFields.emplace(f, newName);
            } else {
...
bool InspectTunaProgram::preorder(const IR::Parameter *param) {
    ...
    auto st = ft->to<IR::Type_StructLike>();
    // check if it is tuna specific standard metadata
    ...
    if (TunaProgramStructure::isStandardMetadata(ptName)) {
        LOG5("Parameter is tuna standard metadata");
        addHeaderType(st);
...
void InspectTunaProgram::postorder(const IR::P4Parser *p) {
...
void InspectTunaProgram::postorder(const IR::P4Control *c) {
    if (pinfo->block_type.count(c)) {
        ...
        else if (info.first == INGRESS && info.second == VERIFY_CHECKSUM)
            pinfo->verify_checksums.emplace("ingress"_cs, c);
        else if (info.first == INGRESS && info.second == COMPUTE_CHECKSUM)
            pinfo->compute_checksums.emplace("ingress"_cs, c);
        else if (info.first == EGRESS && info.second == VERIFY_CHECKSUM)
            pinfo->verify_checksums.emplace("egress"_cs, c);
        else if (info.first == EGRESS && info.second == COMPUTE_CHECKSUM)
            pinfo->compute_checksums.emplace("egress"_cs, c);
    }
}

bool ParseTunaArchitecture::preorder(const IR::ExternBlock *block) {
    if (block->node->is<IR::Declaration>()) structure->globals.push_back(block);
    return false;
}

bool ParseTunaArchitecture::preorder(const IR::PackageBlock *block) {
    ...
    if (auto ingress = pkg->to<IR::PackageBlock>()) {
        ...
        p = ingress->findParameterValue("vr"_cs);
        if (p == nullptr) {
            modelError("'ingress' package %1% has no parameter named 'vr'", block);
            return false;
        }
        auto verifyChecksum = p->to<IR::ControlBlock>();
        if (verifyChecksum == nullptr) {
            modelError("%1%: 'vr' argument of 'ingress' should be bound to a control", block);
            return false;
        }
        ...
        p = ingress->findParameterValue("ck"_cs);
        if (p == nullptr) {
            modelError("'ingress' package %1% has no parameter named 'ck'", block);
            return false;
        }
        auto computeChecksum = p->to<IR::ControlBlock>();
        if (computeChecksum == nullptr) {
            modelError("%1%: 'ck' argument of 'ingress' should be bound to a control", block);
            return false;
        }
        ...
        structure->block_type.emplace(verifyChecksum->container,
                                      std::make_pair(INGRESS, VERIFY_CHECKSUM));
        structure->block_type.emplace(pipeline->container, std::make_pair(INGRESS, PIPELINE));
        structure->block_type.emplace(computeChecksum->container,
                                      std::make_pair(INGRESS, COMPUTE_CHECKSUM));
        ...
        structure->non_pipeline_controls.emplace(verifyChecksum->container->name);
        structure->non_pipeline_controls.emplace(computeChecksum->container->name);
    ...
    if (auto egress = pkg->to<IR::PackageBlock>()) {
        ...
        p = egress->findParameterValue("vr"_cs);
        if (p == nullptr) {
            modelError("'egress' package %1% has no parameter named 'vr'", block);
            return false;
        }
        auto verifyChecksum = p->to<IR::ControlBlock>();
        if (verifyChecksum == nullptr) {
            modelError("%1%: 'vr' argument of 'egress' should be bound to a control", block);
            return false;
        }
        ...
        auto pipeline = p->to<IR::ControlBlock>();
        if (pipeline == nullptr) {
            modelError("%1%: 'eg' argument of 'egress' should be bound to a control", block);
            return false;
        }

        p = egress->findParameterValue("ck"_cs);
        if (p == nullptr) {
            modelError("'egress' package %1% has no parameter named 'ck'", block);
            return false;
        }
        auto computeChecksum = p->to<IR::ControlBlock>();
        if (computeChecksum == nullptr) {
            modelError("%1%: 'ck' argument of 'egress' should be bound to a control", block);
            return false;
        }
