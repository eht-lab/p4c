...
#include "backends/apollo/common/check_unsupported.h"
#include "backends/apollo/tuna_nic/options.h"
...
/// The policy is: convert all enums that are not part of the tuna.
/// Use 32-bit values for all enums.
/// Also convert TUNA_PacketPath_t to bit<32>
class TunaEnumOn32Bits : public P4::ChooseEnumRepresentation {
    cstring filename;

    bool convert(const IR::Type_Enum *type) const override {
        if (type->name == "TUNA_PacketPath_t") return true;
        if (type->name == "TUNA_Ingress_dest_t") return true;
        if (type->name == "TUNA_Egress_dest_t") return true;
 ...
 public:
    explicit TunaEnumOn32Bits(cstring filename) : filename(filename) {}
};

TunaNicMidEnd::TunaNicMidEnd(CompilerOptions &options, std::ostream *outStream)
    : PortableMidEnd(options) {
    auto convertEnums = new P4::ConvertEnums(&typeMap, new TunaEnumOn32Bits("tuna.p4"_cs));
    auto evaluator = new P4::EvaluatorPass(&refMap, &typeMap);
    // fix me, is LocalCopyPropagation policy supported for tuna?
    ...
    if (Apollo::TunaNicContext::get().options().loadIRFromJson == false) {
        addPasses({
            ...
            new P4::ValidateTableProperties({
                "tuna_implementation"_cs,
                "tuna_direct_counter"_cs,
                "tuna_idle_timeout"_cs,
                "size"_cs,
            }),
        ...
        auto fillEnumMap = new P4::FillEnumMap(new TunaEnumOn32Bits("tuna.p4"_cs), &typeMap);
        addPasses({
