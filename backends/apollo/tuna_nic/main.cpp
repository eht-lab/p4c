...
#include "backends/apollo/common/JsonObjects.h"
#include "backends/apollo/tuna_nic/midend.h"
#include "backends/apollo/tuna_nic/options.h"
#include "backends/apollo/tuna_nic/tunaNic.h"
#include "backends/apollo/tuna_nic/version.h"
...
int main(int argc, char *const argv[]) {
    setup_gc_logging();
    // TunaNicContext defined in ./options.h
    AutoCompileContext autoTunaNicContext(new Apollo::TunaNicContext);
    auto &options = Apollo::TunaNicContext::get().options();
    options.langVersion = CompilerOptions::FrontendVersion::P4_16;
    options.compilerVersion = cstring(TUNA_NIC_VERSION_STRING);
    ...
    // Apollo is required for compatibility with the previous compiler.
    options.preprocessor_options += " -D__TARGET_APOLLO__";
    ...
    Apollo::TunaNicMidEnd midEnd(options);
    ...
    auto backend =
        new Apollo::TunaNicBackend(options, &midEnd.refMap, &midEnd.typeMap, &midEnd.enumMap);

    AutoCompileContext autoContext(new Apollo::TunaNicContext(Apollo::TunaNicContext::get()));
    try {
        backend->convert(toplevel);  // call tunaNic.cpp:TunaNicBackend::convert()
    } catch (const std::exception &bug) {
        std::cerr << bug.what() << std::endl;
        return 1;
    }
