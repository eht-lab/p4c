...
#ifndef BACKENDS_APOLLO_COMMON_CONTROL_H_
#define BACKENDS_APOLLO_COMMON_CONTROL_H_
...
class ControlConverter : public Inspector {
 ...
 protected:
    Util::IJson *convertTable(const CFG::TableNode *node, Util::JsonArray *action_profiles,
                              Apollo::SharedActionSelectorCheck<arch> *selector_check) {
        ...
        if (key != nullptr) {
            ...
            for (auto ke : key->keyElements) {
                ...
                if (match_type == Apollo::MatchImplementation::selectorMatchTypeName) continue;
                // Decreasing order of precedence (apollo specification):
                ...
                if (match_type != table_match_type) {
                    if (match_type == Apollo::MatchImplementation::rangeMatchTypeName)
                        table_match_type = Apollo::MatchImplementation::rangeMatchTypeName;
                    if ((match_type == corelib.ternaryMatch.name ||
                         match_type == Apollo::MatchImplementation::optionalMatchTypeName) &&
                        table_match_type != Apollo::MatchImplementation::rangeMatchTypeName)
                ...
                // represented in the Apollo JSON file the same as a ternary
                // field would be.
                if (match_type == Apollo::MatchImplementation::optionalMatchTypeName) {
                ...
                if (auto na = ke->getAnnotation(IR::Annotation::nameAnnotation)) {
                    ...
                    // This is a Apollo JSON extension: specify a
        ...
        if (size == 0) size = Apollo::TableAttributes::defaultTableSize;
        ...
        // 处理 tuna_direct_counter
        auto tuna_ctrs = table->properties->getProperty("tuna_direct_counter");
        if (tuna_ctrs != nullptr) {
            if (tuna_ctrs->value->is<IR::ExpressionValue>()) {
                auto expr = tuna_ctrs->value->to<IR::ExpressionValue>()->expression;
                if (expr->is<IR::PathExpression>()) {
                    auto pe = expr->to<IR::PathExpression>();
                    auto decl = ctxt->refMap->getDeclaration(pe->path, true);
                    if (!decl->is<IR::Declaration_Instance>()) {
        ...
        // 处理 tuna_direct_meter
        auto tuna_dm = table->properties->getProperty("tuna_direct_meter");
        if (tuna_dm != nullptr) {
            if (tuna_dm->value->is<IR::ExpressionValue>()) {
                auto expr = tuna_dm->value->to<IR::ExpressionValue>()->expression;
                if (expr->is<IR::PathExpression>()) {
                    auto pe = expr->to<IR::PathExpression>();
                    auto decl = ctxt->refMap->getDeclaration(pe->path, true);
                    if (!decl->is<IR::Declaration_Instance>()) {
        ...
        bool tuna_sup_to = false;
        auto tuna_timeout = table->properties->getProperty("tuna_idle_timeout");
        if (tuna_timeout != nullptr) {
            if (tuna_timeout->value->is<IR::ExpressionValue>()) {
                auto expr = tuna_timeout->value->to<IR::ExpressionValue>()->expression;
                if (!expr->is<IR::BoolLiteral>()) {
                    ::P4::error(ErrorType::ERR_EXPECTED, "%1%: must true/false", tuna_timeout);
                } else {
                    tuna_sup_to = expr->to<IR::BoolLiteral>()->value;
                }
            } else {
                ::P4::error(ErrorType::ERR_EXPECTED, "%1%: expected a Boolean", tuna_timeout);
            }
        }

        if (!result->get("support_timeout")) {
            result->emplace("support_timeout", tuna_sup_to);
        }
        ...
        for (auto e : entriesList->entries) {
            ...
            for (auto k : keyset->components) {
                ...
                if (matchType == corelib.exactMatch.name) {
                    ...
                    // the Apollo JSON file as table key fields with
    ...
    bool handleTableImplementation(const IR::Property *implementation, const IR::Key *key,
                                   ...
                                   Apollo::SharedActionSelectorCheck<arch> *) {
        ...
        if (propv->expression->is<IR::ConstructorCallExpression>()) {
            ...
            if (implementationType->name == actionSelectorName) {
                ...
                for (auto ke : key->keyElements) {
                    ...
                    BUG_CHECK(mt != nullptr, "%1%: could not find declaration", ke->matchType);
                    if (mt->name.name != Apollo::MatchImplementation::selectorMatchTypeName)
                        continue;
 ...
 public:
    ...
    bool preorder(const IR::P4Control *cont) override {
        ...
        auto selector_check = new Apollo::SharedActionSelectorCheck<arch>(ctxt);
...
#endif /* BACKENDS_APOLLO_COMMON_CONTROL_H_ */
