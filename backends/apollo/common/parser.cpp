    ...
    if (stat->is<IR::AssignmentStatement>()) {
        ...
        if (minst->is<P4::ExternMethod>()) {
            ...
            if (extmeth->method->name.name == corelib.packetIn.extract.name) {
                ...
                std::stringstream ss;
                value->serialize(ss);
                LOG1(__func__ << ":" << __LINE__ << " type=" << type
                              << " value content=" << ss.str());
...
unsigned ParserConverter::combine(const IR::Expression *keySet, const IR::ListExpression *select,
    ...
    // From the Apollo spec: For values and masks, make sure that you
...
Util::IJson *ParserConverter::stateName(IR::ID state) { return new Util::JsonValue(state.name); }
