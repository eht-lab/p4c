...
void populateSelectiveAbseilArgsImpl(const Tuple &tup, std::vector<absl::FormatArg> &abslArgs,
                                     std::index_sequence<Is...>) {
        ...
        (
            [&] {
                ...
                if constexpr (std::is_integral_v<ElementType>) {
                    ...
                    abslArgs.push_back(absl::FormatArg(tempSs.str()));
                }
            }(),
            0)...  // 0 is placeholder for initializer list expansion
    };
...
template <typename Tuple>
void populateSelectiveAbseilArgs(const Tuple &tup, std::vector<absl::FormatArg> &abslArgs) {
    populateSelectiveAbseilArgsImpl(tup, abslArgs,
...
std::string formatAbslStyle(std::string_view formatSv, const Tuple &argTuple) {
    ...
    populateSelectiveAbseilArgs(argTuple, abslArgs);
